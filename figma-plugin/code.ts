...
function setTextFont(textNode: any, style: string = FONT_STYLE_REGULAR) {
...
// Component Creators
function createButton(text: string, variant: 'primary' | 'secondary' | 'outline' = 'primary', width: number = 200): any {
...
function createInput(label: string, placeholder: string, width: number = 340): any {
...
function createStatCard(value: string, label: string, icon: string, color: any = COLORS.primary): any {
  const card = figma.createFrame();
...
function createBadge(text: string, color: any, bgColor: any): any {
...
function createProgressBar(value: number, maxValue: number, color: any, width: number = 300): any {
  const container = figma.createFrame();
...
function createTableRow(col1: string, col2: string, col3: string, col4: string, statusColor: any = COLORS.success): any {
...
async function generateLandingPage(frame: any) {
...
async function generateSignupPage(frame: any) {
